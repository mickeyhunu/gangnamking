...
- 검색 엔진(네이버/구글/다음/빙/야후/덕덕고/카카오) 또는 동일 도메인에서 넘어오는 정상적인 트래픽만 허용하고 싶다면 `REFERRER_ALLOWED_HOSTS` 환경 변수로 허용할 도메인 목록을 콤마로 지정할 수 있습니다. 설정하지 않으면 기본 검색엔진 목록을 사용하며, 주소창 직접 입력·북마크로 접근한 요청도 통과됩니다.
