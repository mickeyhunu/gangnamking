...
function findShopByIdentifier(shops, identifier) {
  if (!Array.isArray(shops)) {
    return { shop: null, translationLang: null };
  }
  ...
  for (const shop of shops) {
    ...
    if (shop.id === normalizedIdentifier) {
      return { shop, translationLang: null };
    }
    ...
    for (const [lang, translation] of Object.entries(translations)) {
      ...
      const translationId = typeof translation.id === 'string' ? translation.id.trim() : '';

      if (translationId && translationId === normalizedIdentifier) {
        return { shop, translationLang: lang };
      }
    }
  }
